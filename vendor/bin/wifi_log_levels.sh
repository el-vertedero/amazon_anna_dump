#!/vendor/bin/sh
#
# Copyright (c) 2017 - 2025 Amazon.com, Inc. or its affiliates.  All rights reserved.
...
function iwpriv_get_channel_bw
{
    GET_CNM_OUT=`$IWPRIV $WLAN_INTERFACE driver get_cnm | $GREP -Eaie "channels|ConnStat"`
    CHANNEL=`echo "$GET_CNM_OUT" | $GREP channels | cut -d " " -f4`
    BW=`echo "$GET_CNM_OUT" | $GREP ConnStat | awk -F'[][]' '{for(i=1;i<=NF;i++) if($i~/^BW/) {split($i,a," "); print a[2]; exit}}' `
    if [ -z "$BW" ] ; then
        BW=20
    fi
    BANDWIDTH=$BW"MHz"
